...
def test_consistent_versions(repository: Path, scripts: dict[str, ModuleType]) -> None:
    ...
    assert checker.repository_version() == "1.2.3"
    assert checker.version_errors("1.2.3", "v1.2.3") == []


@pytest.mark.parametrize("version", ["1.2", "01.2.3", "1.2.3-rc1", "garbage", ""])
def test_invalid_canonical_version(
    ...
    with pytest.raises(ValueError, match=r"must contain X\.Y\.Z"):
        scripts["check_versions"].repository_version()
...
def test_stale_metadata(
    ...
    assert any(
        relative_path in error for error in scripts["check_versions"].version_errors("1.2.3")
    )


def test_missing_locked_projects(repository: Path, scripts: dict[str, ModuleType]) -> None:
    (repository / "uv.lock").write_text("package = []\n", encoding="utf-8")
    errors = scripts["check_versions"].version_errors("1.2.3")
    assert len(errors) == len(scripts["check_versions"].PROJECT_FILES)
...
def test_wrong_tag(repository: Path, scripts: dict[str, ModuleType]) -> None:
    assert scripts["check_versions"].version_errors("1.2.3", "v1.2.2") == [
...
def test_set_version_success(
    ...
    assert scripts["check_versions"].repository_version() == "2.0.0"
    assert scripts["check_versions"].version_errors("2.0.0") == []


@pytest.mark.parametrize("failure", ["lock", "validation", "declaration"])
def test_failed_update_restores_every_managed_file(
    ...
    run = Mock(side_effect=fail_lock if failure == "lock" else None)
    monkeypatch.setattr(setter.subprocess, "run", run)
    assert setter.main() == 1
