...
def test_message_file(
    ...
    assert scripts["check_commits"].main() == expected
...
def test_range_excludes_existing_history_and_reports_every_failure(
    ...
    assert scripts["check_commits"].main() == 0
    ...
    assert scripts["check_commits"].main() == 1
...
def test_missing_message_file_fails_closed(
    scripts: dict[str, ModuleType], monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(
