...
uv resolves the workspace together; selecting `uv run --package example-package-a` does not create
...
```bash
uv add --package example-package-a 'httpx>=0.28,<1'
uv add --group docs 'sphinx>=9.1,<10'
uv tree --package example-package-a
