...
def record(path: Path, *, root: Path | None = None) -> None:
...
def required_ci_jobs(*, root: Path | None = None) -> set[str]:
    ...
    required = {
        check["context"]
        for check in load_policy(root=root)["protection"]["required_status_checks"]["checks"]
    }
...
def verify_commit(
    repository: str, commit: str, required: set[str], workflow_id: int, *, root: Path | None = None
) -> list[str]:
    ...
    check_evidence(repository, run["id"], pr["head"]["sha"], commit, root=root)
